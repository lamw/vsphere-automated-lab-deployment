...
$NestedESXiApplianceOVA = 'C:\Store\Software\ISO\VMware\Nested_ESXi6.7_Appliance_Template_v1\Nested_ESXi6.7_Appliance_Template_v1.ovf'
$VCSAInstallerPath = 'C:\Store\Software\ISO\VMware\VMware-VCSA-all-6.7.0-10244745'
...
$NestedESXiHostnameToIPs = @{
    "vesxi01" = "$($SubnetPrefix).51"
    "vesxi02" = "$($SubnetPrefix).52"
    "vesxi03" = "$($SubnetPrefix).53"
}
...
$VCSAIPAddress = "$($SubnetPrefix).50"
$VCSAHostname = "vcsa.lab.local" #Change to IP if you don't have valid DNS
$VCSAPrefix = "24"
$VCSASSODomainName = "vsphere.local"
$VCSASSOPassword = "Pa55word!"
$VCSARootPassword = "Pa55word!"
$VCSASSHEnable = "true"

# General Deployment Configuration for Nested ESXi, VCSA & NSX VMs
$VirtualSwitchType = "VSS" # VSS or VDS
$VMNetwork = "VM Network"
$VMDatastore = "datastore01"
$VMNetmask = "255.255.255.0"
$VMGateway = "$($SubnetPrefix).1"
$VMDNS = "$($SubnetPrefix).30"
$VMNTP = "pool.ntp.org"
$VMPassword = "Pa55word!"
$VMDomain = "lab.local"
$VMSyslog = "$($SubnetPrefix).30"
...
$NSXHostname = "nsx63-1.lab.local"
$NSXIPAddress = "$($SubnetPrefix).250"
$NSXNetmask = "255.255.255.0"
$NSXGateway = "$($SubnetPrefix).1"
$NSXSSHEnable = "true"
$NSXCEIPEnable = "false"
$NSXUIPassword = "Pa55word!"
$NSXCLIPassword = "Pa55word!"
...
$vcsaSize2MemoryStorageMap = @{
    "tiny"   = @{"cpu" = "2"; "mem" = "10"; "disk" = "300"};
    "small"  = @{"cpu" = "4"; "mem" = "16"; "disk" = "340"};
    "medium" = @{"cpu" = "8"; "mem" = "24"; "disk" = "525"};
    "large"  = @{"cpu" = "16"; "mem" = "32"; "disk" = "740"};
    "xlarge" = @{"cpu" = "24"; "mem" = "48"; "disk" = "1180"}
}
...
if ($DeploymentTarget -eq "ESXI") {
    ...
    if ($VirtualSwitchType -eq "VSS") {
        ...
        if ($DeployNSX -eq 1) {
            $privateNetwork = Get-VirtualPortGroup -Server $viConnection -Name $PrivateVXLANVMNetwork | Select-Object -First 1
        }
    } else {
        $network = Get-VDPortgroup -Server $viConnection -Name $VMNetwork | Select-Object -First 1
        if ($DeployNSX -eq 1) {
            $privateNetwork = Get-VDPortgroup -Server $viConnection -Name $PrivateVXLANVMNetwork | Select-Object -First 1
        }
    }
...
if ($deployNestedESXiVMs -eq 1) {
    if ($DeploymentTarget -eq "ESXI") {
        ...
        $NestedESXiHostnameToIPs.GetEnumerator() | Sort-Object -Property Value | Foreach-Object {
            ...
            $networkMapLabel = ($ovfconfig.ToHashTable().keys | Where-Object {$_ -Match "NetworkMapping"}).replace("NetworkMapping.", "").replace("-", "_")
